...
def test_ci_dependency_free_runner_uses_source_tree() -> None:
    ...
    assert "NETOPS_REQUIRE_RUNTIME_TESTS=1 python -m pytest -q" in workflow
...
def _replace_current_changelog_heading(path: Path, version: str) -> None:
    text = path.read_text(encoding="utf-8")
    pattern = re.compile(r"^## 0\.2\.1(?P<suffix> - [^\n]+)$", re.MULTILINE)
    path.write_text(
...
def test_version_invariant_rejects_changed_compose_image(tmp_path: Path) -> None:
    ...
    _replace_exact(
        root / "compose.yaml",
        "image: local/netops-helper:0.2.1",
        "image: local/netops-helper:0.2.2",
    )
    errors = _load_release_module("check_public_release")._version_invariant_errors(root)
...
def test_version_invariant_rejects_changed_sbom_root(tmp_path: Path) -> None:
    ...
    sbom_path = root / "sbom.cdx.json"
    sbom = json.loads(sbom_path.read_text(encoding="utf-8"))
    sbom["metadata"]["component"]["version"] = "0.2.2"
    sbom_path.write_text(json.dumps(sbom), encoding="utf-8")
    errors = _load_release_module("check_public_release")._version_invariant_errors(root)
...
def test_version_invariant_ignores_backtick_fenced_heading_decoy(
    ...
    root = _version_fixture(tmp_path)
    _replace_current_changelog_heading(root / "CHANGELOG.md", "0.2.2")
    _replace_exact(
        root / "CHANGELOG.md",
        "# Changelog\n\n",
        "# Changelog\n\n```md\n## 0.2.1 - fenced decoy\n```\n\n",
...
def test_version_invariant_respects_tilde_fence_closing_length(
    ...
    _replace_exact(
        ...
        (
            "# Changelog\n\n~~~~~markdown\n"
            "## 0.2.1 - first fenced decoy\n"
            "~~~~\n"
            "## 0.2.1 - still fenced after short closer\n"
            "~~~~~~\n\n"
        ),
    )
    errors = _load_release_module("check_public_release")._version_invariant_errors(root)
...
def test_version_invariant_rejects_wrong_or_duplicate_changelog_heading(
    ...
    _replace_current_changelog_heading(wrong / "CHANGELOG.md", "0.2.2")
...
def _public_source_fixture(tmp_path: Path) -> Path:
    ...
    exported = output / "netops-helper-0.2.1"
    manifest = json.loads(
...
def test_release_export_preserves_existing_destination(tmp_path: Path) -> None:
    ...
    destination = output / "netops-helper-0.2.1"
...
def test_release_export_rejects_symlinked_output(tmp_path: Path) -> None:
    ...
    cases = {
        "direct": (direct_link, actual / "netops-helper-0.2.1"),
        "parent": (
...
def test_public_export_contains_no_python_bytecode(tmp_path: Path) -> None:
    ...
    exported = tmp_path / "netops-helper-0.2.1"
...
def test_runtime_image_drops_pip_and_carries_oci_labels() -> None:
    ...
    assert 'org.opencontainers.image.source="https://github.com/radek-cerny-soukr/netops-helper"' in dockerfile
...
def test_gate_flags_credential_shaped_and_private_network_markers() -> None:
    ...
    flagged = {
        ...
        "next hop fe80::1234:abcd": "private network marker",
        "peer fd12:3456::1": "private network marker",
        "carrier 100.64.3.7": "private network marker",
        "resolver nas.local": "private network marker",
    }
    ...
    assert gate._secret_and_network_marker_errors("tests/fixture.py", "peer fe80::1") == []


def test_gate_rejects_tracked_or_unignored_files_outside_allowlist() -> None:
    gate = _load_release_module("check_public_release")
    with tempfile.TemporaryDirectory() as directory:
        ...
        subprocess.run(["git", "-C", str(root), "add", "README.md", "notes.txt"], check=True)
        (root / "vault.json").write_text("{}\n", encoding="utf-8")
        errors = gate._tracked_outside_allowlist_errors(root, {"README.md"})
        assert errors == [
            "tracked or unignored file outside release allowlist: notes.txt",
            "tracked or unignored file outside release allowlist: vault.json",
        ]
        (root / ".gitignore").write_text("vault.json\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(root), "add", ".gitignore"], check=True)
        assert gate._tracked_outside_allowlist_errors(root, {"README.md", ".gitignore"}) == [
            "tracked or unignored file outside release allowlist: notes.txt",
        ]
    assert gate._tracked_outside_allowlist_errors(Path(directory), set()) == []
...
def main() -> int:
    ...
    test_github_actions_use_full_commit_shas()
    test_ci_dependency_free_runner_uses_source_tree()
    ...
    test_gate_rejects_tracked_or_unignored_files_outside_allowlist()
